...
持续同步:: 初始数据同步后、Cloud Sync 将侦听源S3存储分段上的更改、并在发生更改时持续同步目标。无需按计划间隔重新扫描源。
+
--
只有在创建同步关系以及从S3存储分段同步到S3、Google Cloud Storage、Azure Blob Storage、StorageGRID 或IBM存储时、此设置才可用。

如果启用此设置、则会影响以下其他功能：

** 同步计划已禁用。
** 以下设置将还原为其默认值：Sync Timeout、Recently Modified Files和Date Modified。
** 按大小筛选仅在复制事件(而不是删除事件)上处于活动状态。
